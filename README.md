...
Version 0.1.4.13 retains the Windows Phone picker and share-target registration from 0.1.4.7 and fixes Quick Share control-payload framing, endpoint discovery metadata, P-256 public-key encoding, plaintext connection-response ordering, and file payload identity. Outgoing introductions now use fresh positive IDs consistently in `FileMetadata.payload_id`, `FileMetadata.id`, and each FILE header, and include the file name required by current Android clients. Incoming files accept either interoperable attachment identity field when it is unambiguous, report the received payload details on a mismatch, and complete the bounded safe-disconnection exchange so the sender cannot race the receiver's socket teardown. Encrypted keep-alive and file frames still assign sequence numbers in the same order they are written. The regular-launch action remains `Select file`; Windows 10 Mobile uses the WpBlueBubbles asynchronous multiple-file picker broker, while desktop Windows uses the asynchronous single-file picker API. Microsoft Nearby Share is skipped on Windows 10 Mobile because that Windows release never shipped the PC-only Nearby Sharing feature.
