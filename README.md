...
Version 0.1.4.7 adds the persisted broadcast hostname and About version display from 0.1.4.6, renames the regular-launch action to `Select file`, and uses the asynchronous multiple-file picker broker used by WpBlueBubbles on Windows 10 Mobile. The phone picker starts in Photos and accepts common photo, video, and PDF types; desktop Windows uses the asynchronous single-file picker API. Share-target activation initializes the page before activating the window, and the manifest advertises the same storage formats to the Windows share sheet. Duplicate advertisements for the same recipient and transport are merged into one peer row. Quick Share peers are identified by host name, and LiveDrop filters its own local addresses so a second local instance is not offered as a remote client. Microsoft Nearby Share is skipped on Windows 10 Mobile because that Windows release never shipped the PC-only Nearby Sharing feature; Quick Share remains available there. On desktop Windows, the built-in Connected Devices service can occupy TCP 5040/UDP 5050 and prevent LiveDrop's Microsoft Nearby listener from starting; the app reports this alongside Quick Share status.
