...
Version 0.1.4.6 adds a persisted alphanumeric broadcast hostname with immediate discovery restart and shows the installed version in About. Windows 10 Mobile uses picker continuation activation because its platform does not support `PickSingleFileAsync`; desktop Windows uses the asynchronous picker API. Quick Share peers are identified by host name, and LiveDrop filters its own local addresses so a second local instance is not offered as a remote client. Microsoft Nearby Share is skipped on Windows 10 Mobile because that Windows release never shipped the PC-only Nearby Sharing feature; Quick Share remains available there. On desktop Windows, the built-in Connected Devices service can occupy TCP 5040/UDP 5050 and prevent LiveDrop's Microsoft Nearby listener from starting; the app reports this alongside Quick Share status.
