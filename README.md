...
Version 0.1.4.12 retains the Windows Phone picker and share-target registration from 0.1.4.7 and fixes Quick Share control-payload framing, endpoint discovery metadata, P-256 public-key encoding, and the plaintext connection-response ordering. Encrypted keep-alive and file frames now assign sequence numbers in the same order they are written, so the receiver cannot reject a valid file stream when both tasks are active. Incoming LiveDrop sessions accept by default, show progress and notifications, save completed files in the visible `LiveDrop` folder under Pictures, Videos, or Documents, and close cleanly after the final file without waiting for an optional disconnection frame. The receiver also keeps the interoperable handshake order used by NearDrop and official Quick Share clients. The regular-launch action remains `Select file`; Windows 10 Mobile uses the WpBlueBubbles asynchronous multiple-file picker broker, while desktop Windows uses the asynchronous single-file picker API. Microsoft Nearby Share is skipped on Windows 10 Mobile because that Windows release never shipped the PC-only Nearby Sharing feature.
