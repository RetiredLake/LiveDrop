...
Version 0.1.4.9 retains the Windows Phone picker and share-target registration from 0.1.4.7 and fixes a Quick Share transfer stream race. Encrypted keep-alive and file frames now assign sequence numbers in the same order they are written, so the receiver cannot reject a valid file stream when both tasks are active. The receiver also keeps the interoperable handshake order used by NearDrop and official Quick Share clients. The regular-launch action remains `Select file`; Windows 10 Mobile uses the WpBlueBubbles asynchronous multiple-file picker broker, while desktop Windows uses the asynchronous single-file picker API. Microsoft Nearby Share is skipped on Windows 10 Mobile because that Windows release never shipped the PC-only Nearby Sharing feature.
