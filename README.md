...
Version 0.1.4.8 retains the Windows Phone picker and share-target registration from 0.1.4.7 and fixes the Quick Share transfer handshake. The receiver now sends its connection response and paired-key frame before waiting for the sender's replies, matching the interoperable client/server order used by NearDrop and official Quick Share clients. This removes the pre-introduction wait that caused transfers to fail before consent or file payloads could begin. The regular-launch action remains `Select file`; Windows 10 Mobile uses the WpBlueBubbles asynchronous multiple-file picker broker, while desktop Windows uses the asynchronous single-file picker API. Microsoft Nearby Share is skipped on Windows 10 Mobile because that Windows release never shipped the PC-only Nearby Sharing feature.
