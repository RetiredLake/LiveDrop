...
Version 0.1.4.17 retains the Windows Phone picker and share-target registration from 0.1.4.7 and fixes Quick Share control-payload routing and setup-frame ordering. Incoming BYTES payloads are now reassembled and consumed as sharing-layer control traffic, including cancellation, and setup reads wait for their expected sharing frame so an initial Android progress frame cannot be misread as key or file metadata. Loopback and local-interface transfers are rejected by the production adapters and connector; a standalone test performs the only permitted LiveDrop-to-LiveDrop loopback transfer. The release also retains the 0.1.4.13 fixes for endpoint discovery metadata, P-256 public-key encoding, plaintext connection-response ordering, file payload identity, safe disconnection, and keep-alive framing. The regular-launch action remains `Select file`; Windows 10 Mobile uses the WpBlueBubbles asynchronous multiple-file picker broker, while desktop Windows uses the asynchronous single-file picker API. Microsoft Nearby Share is skipped on Windows 10 Mobile because that Windows release never shipped the PC-only Nearby Sharing feature.
